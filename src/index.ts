...
class DingTalk {
  private readonly app: Electron.App = app
  private readonly ipcMain: Electron.IpcMain = ipcMain
  // 应用窗体
  ...
  private onReady () {
    this.app.on('ready', () => {
      // 同时只能运行一个人实例
      const shouldQuit: boolean = this.app.makeSingleInstance(() => {
        if (this.$window) {
          this.$window.show()
          this.$window.focus()
        }
